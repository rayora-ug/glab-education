...
Adding this required a `Code.gs` redeploy (six new actions: `getRegistrationStatus`, `adminSetRegistrationOpen`, `adminSetStudentBlocked`, `adminFindStudent`, `adminListSubmittedRegistrations`, `adminConfirmRegistration`), plus later three more (`submitInterest`, `adminListInterest`, `adminApproveInterest`) for Next Level Interest.
