...
**`Registrations` is permanent — never clear it between sessions.** Every registration a student has ever made lives here forever, side by side across every batch and every level (A1 rows next to A2 rows next to B1 rows, old sessions next to new ones) — this is not a "current session" working table that needs resetting to prepare for the next one. A new session's registrations are simply new rows appended on top; nothing about old rows gets in the way of new ones. This data also directly powers MyGLAB's **Course History** (a student's full A1 → A2 → B1 journey, straight from these rows) — clearing it erases that history for real, for every student, not just the current session's.

If clearing it ever felt necessary because a student with an old Confirmed registration (say, A1) couldn't get a new one (A2) to go through — that was a real bug in `submitRegistration_`'s duplicate-prevention check, now fixed: it used to treat *any* past registration, for any course, as "already registered," and would silently hand back the old one instead of creating the new row. It's now scoped to the exact batch being submitted, so an old registration for a different course never blocks a new one.
...
**Prefer confirming through `/admin`, not by editing the cell.** Only the admin panel's Confirm button also emails the student their confirmation (batch, start date, WhatsApp/Classroom/Meet links if set) — this is what tells them they're actually in and class links are up, so they don't just keep waiting for an email that never arrives. Editing the `Status` cell directly still works for the student-facing side (portal/MyGLAB update immediately either way) but skips the email entirely.

**Where the email address comes from.** `/portal` and `/results` both now ask for an email address as a required field on the registration form itself, right alongside payment method and proof — so it's captured directly from the student, once, at the moment they register, and lands in that same `Registrations` row's `Email` column. Confirming that row (via the `/admin` Confirm button) reads the email straight off it — no separate lookup, nothing to keep in sync, no manual data entry on your end at all. The only gap: registrations submitted *before* this field existed won't have an email on file — those just silently skip the confirmation email, same as any other missing-email case, and nothing needs backfilling since every registration from here on captures its own.

**Who it sends from.** The confirmation email sends via `GmailApp` as `info@glabeducation.com`, not the personal Gmail account this script is bound to — but that only works because `info@glabeducation.com` is set up as a verified **"Send mail as"** alias on that account (Gmail Settings → Accounts and Import → Send mail as). If that alias is ever removed or was never set up, sending falls back to failing silently the same way any other email error does (see `LAST_EMAIL_ERROR` below) — the confirmation itself is never affected either way. `GmailApp` needs a different authorization scope (`gmail.send`) than plain `MailApp` did, so switching to it once required going through the authorization flow again — see "Troubleshooting a missing confirmation email" below if that ever needs redoing (e.g. after moving this script to a different Google account).

**Troubleshooting a missing confirmation email.** The send is wrapped so it never blocks a real payment confirmation — which also means a failure is otherwise silent. Three Script Properties (Project Settings → Script Properties in the Apps Script editor) exist specifically to make that diagnosable instead:
- `LAST_CONFIRM_ATTEMPT` — written every time a confirmation runs, before anything can go wrong, showing what email/course/batch it actually read off that row. If this is missing after confirming something, `adminConfirmRegistration_` itself isn't being reached at all (check you're using the `/admin` Confirm button, not editing the cell).
- `LAST_EMAIL_SENT` — written only if the send completed without error.
- `LAST_EMAIL_ERROR` — the exact error message if it threw.

The most likely real error, and the one actually hit while building this: `"You do not have permission to call MailApp.sendEmail"` (or the equivalent for `GmailApp`) — meaning the Google account this script runs under has never granted the mail-sending scope. Simply redeploying a new version does **not** re-trigger that consent screen on its own. The reliable fix: go to [myaccount.google.com/permissions](https://myaccount.google.com/permissions), find this project, remove its access entirely, then in the Apps Script editor run any function once (e.g. `doPost`, via the function dropdown next to Run) — with zero prior grant to fall back on, this forces the full "Authorization required" prompt again, and it needs to be granted just once, not on every deploy.
...
`/reviews` and the homepage spotlight now read from **both** the static `data/reviews.json` (the reviews collected before this tab existed — a frozen historical archive, edited only via a code change) **and** this `Reviews` tab's `Synced` rows, live, merged together on every request. The easiest way to add a new review yourself is the "Add Review" form on `/admin` — it appends a row here already marked `Synced`, so it's live on the site within seconds, no code change or redeploy. A student can also submit their own from MyGLAB's "Share Your Feedback" card — those land unsynced, in the "Pending Reviews" queue on `/admin`, waiting for you to click Publish — see below.
...
A confirmed student's home base: course info + dates (from `Batch Links`), attendance, class links (same three as the confirmation page), a free-text note from you, and a **Course History** list — every course they've ever been `Confirmed` for, oldest first, straight from `Registrations` (no separate tracking needed — someone who started directly at A2 just has one entry, which reads as "Started with"). Any student found in `Students` and not blocked can log in, even with no active course right now (e.g. a past workshop attendee) — they just see a lighter message instead of batch details. Logs in with GLAB ID only — no separate account, same identity as everywhere else on the site. (Exam results are not shown here for now.)

**Once eligible for A2/B1 (repeat or next level), registration happens right here** — the same batch-selection, payment-info, proof-upload, and rules form as `/portal`, submitting to the exact same `submitRegistration_` backend (no separate code path, no separate risk), so a student never needs to leave MyGLAB to actually register. Before eligibility, they instead see the "I'm Interested" request form described under Next Level Interest below.

**This is exactly why `Eligible A1`/`A2`/`B1` on `Students` is the only thing that should ever grant someone the ability to register — never a hand-added row on `Registrations`.** `Registrations` should only ever contain rows that came from an actual `/portal` or MyGLAB submission; a manually-added row there (even with `Status` left blank) will make the site tell that student their registration is real, which it isn't. If you want to grant someone eligibility, always do it on `Students`, either directly or via the `Next Level Interest` approval flow below — never by adding anything to `Registrations` yourself.
...
A few things live here for now:
...
**Next Level Interest.** A student who's confirmed for one level but not yet eligible for the next sees a "Ready for your next level?" card on MyGLAB — either an "I'm Interested" request form (not yet eligible) or the real registration form (already eligible) — and if their *current* level (A2 or B1 only, not A1) also has open registration, a "Want to repeat A2/B1 instead?" checkbox appears below it, letting them fold that same card open into the repeat version instead of moving up. Only one of the two is ever shown at a time — checking the box swaps the card to the repeat level and hides the advance one, unchecking it swaps back — so a student can't confuse an old registration with the new one. Once they submit an interest request or become eligible for one option, that one wins permanently: the card locks to that level and the toggle checkbox disappears, since switching would no longer make sense. Both the interest form and the registration form work identically: pick a batch (shown with schedule and fee) and an email, then submit. Submitting the interest form adds a row to a `Next Level Interest` tab (created automatically on first use — no setup needed) with their GLAB ID, name, requested level, requested batch, and a snapshot of their current batch (for your context when deciding — not looked up fresh later). Once submitted, MyGLAB hides that form on every future login too (not just for the rest of that browser session) — it checks the sheet itself for an unprocessed request, rather than relying on anything the browser remembers. This card lists every unactioned request; clicking **Approve** checks the matching `Eligible A1`/`Eligible A2`/`Eligible B1` box on `Students` for that student and marks the request processed, so it drops off the list — after that MyGLAB shows them the real registration form right there, picking whichever batch is actually still open at that point (the requested batch here is only a preference signal, not a hard reservation). A repeat-A1 request has no self-service path at all (A1 was never registered through `/portal`) — those come to you directly, handled the same manual way as any other A1 admissions decision.

**Approving works two ways, and both actually work now.** Click Approve in `/admin`, *or* just check the `Processed` box yourself directly on the `Next Level Interest` tab — either one flips the matching `Eligible` column on `Students`. (Earlier versions of this feature only wired up the `/admin` button; checking `Processed` by hand used to just mark the row done with no other effect, which looked like approving did nothing at all.) The by-hand path runs via an `onEdit` trigger in `Code.gs` — a "simple trigger," meaning it just works the moment the code is deployed, nothing to set up under Triggers, unlike `refreshRegistrationPending`. It only reacts to a single checkbox being *checked* (not unchecked, and not a multi-cell paste — tick one row at a time by hand, or use `/admin` for anything bulkier).

**If checking `Processed` by hand doesn't seem to grant eligibility:** the trigger deliberately never blocks your edit from saving even if something inside it fails, which also means a failure is otherwise invisible. Check Script Properties (Project Settings, gear icon, in the Apps Script editor) for three keys: `LAST_ONEDIT_ATTEMPT` (written every time, showing exactly what GLAB ID/level it read off that row), `LAST_ONEDIT_SUCCESS` (written only if the `Eligible` flip completed), and `LAST_ONEDIT_ERROR` (the actual error, if any). If `LAST_ONEDIT_ATTEMPT` never appears at all after checking the box, the trigger isn't firing — a code change was deployed but Google occasionally needs the *next* edit after a fresh deployment to pick up an updated simple trigger, so try one more check/uncheck/check cycle.
...
Adding this required a `Code.gs` redeploy (six new actions: `getRegistrationStatus`, `adminSetRegistrationOpen`, `adminSetStudentBlocked`, `adminFindStudent`, `adminListSubmittedRegistrations`, `adminConfirmRegistration`), plus later three more (`submitInterest`, `adminListInterest`, `adminApproveInterest`) for Next Level Interest, and one more (`submitStudentReview`) for Pending Reviews — `listReviews`/`markReviewsSynced` already existed from the original review-syncing setup.
